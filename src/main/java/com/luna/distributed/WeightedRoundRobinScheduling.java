...
/**
 * * Created by zehui on 2017/9/9.
 * <p>
 * 基于权重的轮询调度算法-Java 参考：http://www.cnblogs.com/huligong1234/p/3819979.html
 * 算法流程简介：
 * 在轮询调度的基础上，为每个分配的节点添加权重，算法通过策略选择，
 * 权重越高的ip，被分配到的次数越多
 */
public class WeightedRoundRobinScheduling {
        ...
        while (true) {
            ...
            if (currentIndex == 0) {
                ...
                if (currentWeight <= 0) {
                    ...
                    System.out.println("currentWeight<0,currentWeight:" + currentWeight);
                    if (currentWeight == 0)
                        return null;
                }
            }
    ...
    public void init() {
        ...
        Server s2 = new Server("192.168.0.101", 6);//2
        Server s3 = new Server("192.168.0.102", 9);//6
        Server s4 = new Server("192.168.0.103", 12);//4
        Server s5 = new Server("192.168.0.104", 15);//1
